...
//	private RedisService redisService;
    ...
    public void autoRecovery() {
        ...
        List<Long> favoritesId = favoritesRepository.findIdByName("未读列表");
        List<Collect> collectList = collectRepository.findByCreateTimeLessThanAndIsDeleteAndFavoritesIdIn(date, IsDelete.NO, favoritesId);
        for (Collect collect : collectList) {
            try {
                logger.info("文章id:" + collect.getId());
                collectRepository.modifyIsDeleteById(IsDelete.YES, DateUtils.getCurrentTime(), collect.getId());
    ...
    public void getImageLogoUrl() {
        ...
        for (UrlLibrary urlLibrary : urlLibraryList) {
            try {
                ...
                if (result) {
                    collectRepository.updateLogoUrlByUrl(logoUrl, DateUtils.getCurrentTime(), urlLibrary.getUrl());
